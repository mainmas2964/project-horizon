...
function findRandomInfectPos(origin, dimension, radius = 1) {
    for (let i = 0; i < 10; i++) {
        ...
        else if (block && block.typeId !== "minecraft:air" && i > 5) {
            return pos;
        }
    }
    return null;
}
sculkSystem.registerTaskFactory("sculk_worm_infection_random", (data, resolveTarget) => {
    return (target) => {

        // Остановка если биомасса кончилась или мы ждём и нет target
        if (data.biomass <= 0 || (!target && data.waitingTime < 0)) return false;

        // Если мы в режиме ожидания — уменьшаем таймер
        if (!target) {
            sculkSystem.resolveCurrentData(data.id, ["data", "waitingTime"], data.waitingTime - 1000000);
            return true;
        }

        // Инфицированием блока под target
        infectWithSculk(target, 1);

        // Пытаемся найти следующее место заражения
        let nextPos = findRandomInfectPos(target.location, target.dimension, 2);

        if (nextPos) {
            // Обновляем цель и биомассу
...
export function startSculkInfectRandom(biomass, source, id) {
    sculkSystem.addTask(null, {
        delay: 5,
        customId: id,
        repeat: 1,
        priority: "normal",
        persist: true,
        type: "sculk_worm_infection_random",
        data: {
            biomass: biomass,
            id: id,
            waitingTime: 100000

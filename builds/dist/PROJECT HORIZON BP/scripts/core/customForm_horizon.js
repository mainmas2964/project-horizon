  ...
  show(player, category = this.getKeyAtIndex(0)) {
    ...
    if (buttons) {
      for (const txt of buttons) {
        const namespacedTexture = txt.texture.includes(":") ? txt.texture : `horizon:${txt.texture}`;
        const ID = typeIdToDataId.get(namespacedTexture) ?? typeIdToID.get(namespacedTexture);
